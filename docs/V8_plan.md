...
2. 버전: `VERSION` 환경변수가 있으면 그것, 없으면
   `https://api.github.com/repos/pyjhoop/tui-ssh-client/releases/latest`의 `tag_name`을
...
7. `go install github.com/pyjhoop/tui-ssh-client@latest`로 깐 바이너리의 `--version`이 커밋 해시를
   보여준다(ldflags 폴백).
