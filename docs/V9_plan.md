...
### 4. 클립보드 — 프리픽스로 내보내고 `clipHold` 뒤에 지운다
`a.clip = ansi.SetSystemClipboard(text)`를 세우고 `tea.Tick(clipHold)`로 `clipFlushMsg`를
받아 지운다. `View()`가 그동안의 프레임에 **프리픽스로** 붙인다.

**한 프레임만 유지하면 안 된다**(실제로 그렇게 만들었다가 "드래그 복사가 안 된다"로
돌아왔다). bubbletea v1의 표준 렌더러는 `write()`에서 버퍼를 리셋해 **가장 최근 View 결과
하나만** 들고 있고 60fps 티커로 그린다 — 16ms 안에 만들어졌다 다음 `Update`로 교체된
프레임은 렌더러가 **한 번도 보지 못한다**. 즉시 flush하면 이스케이프가 화면에 나가지 않는다.
`clipHold`(150ms)는 티커가 최소 몇 번은 돌 시간이고, 그 사이 바뀌지 않은 줄은 렌더러가
건너뛰므로 터미널에는 여전히 한 번만 도착한다.
...
4. `internal/ui` 클립보드:
   - `TestCopyEmitsOSC52Once` — 다시 그려도 살아 있고, flush 뒤에는 사라진다.
   - `TestCopyHoldsSequenceForSeveralFrames` — flush가 **즉시 오면 안 된다**(렌더러가
     한 프레임도 못 그린 채 지나간다).
