...
## 향후 MVP 로드맵 (합의 후 상세화)
- **v1**: 세션 안정화 — 스크롤백/리사이즈 견고화, known_hosts 호스트키 검증, 연결 에러 UX, 서버 편집/삭제. 상세는 `docs/V1_plan.md`.
- **v2**: SFTP 파일 브라우저 — 우측을 Local | Remote로 분할, 드래그 앤 드롭 전송. 상세는 `docs/V2_plan.md`.
- **v3**: 다중 세션(탭), 세션 유지/재연결, 검색/필터, 그룹/폴더. SFTP 재귀 전송·진행률.
- **v4**: 보안 — 비밀번호 OS 키체인 저장, `~/.ssh/config` import, ssh-agent/점프호스트.
- **v5**: 편의 — 포트포워딩, 테마/키맵 설정.
