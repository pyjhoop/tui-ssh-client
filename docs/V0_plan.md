...
- **v7**: 키맵 단일화 — 바인딩 레지스트리, `?` 도움말 오버레이, 조립되는 상태줄 힌트,
  `keys.json` 재바인딩. 상세는 `docs/V7_plan.md`. (구현 완료)
  (v0의 키 6개가 40개를 넘었는데 목록이 어디에도 없어서, 연결 계층 작업보다 이걸 먼저 한다.)
- **v8**: 연결 편의 — 포트포워딩(`-L`/`-R`), 점프호스트/ProxyJump, 커맨드 팔레트, 테마 설정.
