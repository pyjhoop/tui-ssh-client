...
세션 패널에서 **드래그하면 선택되고, 버튼을 떼는 순간 시스템 클립보드로 복사됩니다**
(`y` 같은 두 번째 키가 없는 이유는 세션 안의 모든 키가 원격 셸의 것이기 때문입니다).
스크롤백으로 올라가서 긁으면 그 과거 줄이 복사되고, 화면을 스크롤하거나 창 크기를 바꾸면
선택은 풀립니다 — 그 행이 다른 내용이 되기 때문입니다. 아주 긴 선택은 64 KiB에서 잘리고
상태줄이 그렇게 알립니다.

붙여넣기는 따로 없습니다. 터미널의 bracketed paste가 그대로 세션으로 들어갑니다.

> **어디로 나가는지 알고 쓰세요.** 복사는 OSC 52 이스케이프로 **우리를 감싼 터미널**에
> 넘겨집니다. tmux 안이라면 `set -g set-clipboard on`이 없으면 버려지고, 중첩 ssh를
> 거친다면 그 경로 전부를 지나갑니다. 우리가 검증할 수 없는 구간이므로 —
> **비밀번호를 긁으면 그 경로로 나갑니다.** 마스킹하는 척하지 않습니다.
> 반대로 클립보드를 **읽지는** 않습니다(OSC 52 질의를 보내지 않습니다).

원격 앱(vim 등)이 마우스 리포팅을 켜 두었더라도 드래그는 **우리 선택이 가져갑니다** —
화면의 글자를 긁는 것이 터미널에 기대하는 기본 동작이기 때문입니다. 그 앱의 마우스가
필요하면 앱 자체의 키보드 조작을 쓰세요.
