...
`docs/V7_plan.md`(키맵 레지스트리·`?` 도움말·`keys.json`) 참조 — 구현 결정이 바뀌면 그 문서도 같이 갱신할 것.

## 현재 상태
v0 + v1 + v2 + v3 + v4 + v5 + v6 + v7 구현 완료. `go build`/`go test -race`/`go vet` 모두 통과한다.
...
```
main.go                    플래그(`--pull`/`--repo`/`--path`/`--keys`) → 부트스트랩 → tea.NewProgram(...)
