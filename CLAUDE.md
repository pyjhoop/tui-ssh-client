...
`docs/V6_plan.md`(암호화 금고·ssh-agent·비공개 레포 동기화) 참조 — 구현 결정이 바뀌면 그 문서도 같이 갱신할 것.

## 현재 상태
v0 + v1 + v2 + v3 + v4 + v5 + v6 구현 완료. `go build`/`go test -race`/`go vet` 모두 통과한다.
...
```
main.go                    플래그(`--pull`/`--repo`/`--path`) → 부트스트랩 → tea.NewProgram(...)
...
internal/ssh/session.go    Dial → RequestPty → Shell, stdout 펌프, WindowChange, keepalive,
                           authMethods(password/key/agent) + keySigner/agentSigners
...
- 루트 model의 상태는 `focus`(sidebar|form|session|local|remote|import|sync)와
  ...
  잠금 화면의 것이다. **키 라우팅은 항상 `focus` 기준**이며,
