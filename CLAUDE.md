...
- **클립보드는 쓰기만 한다.** `ansi.SetSystemClipboard`를 `App.clip`에 세우고 `View`가 그
  프레임에 **프리픽스로 한 번** 내보낸 뒤 `clipFlushMsg`가 지운다 — 시퀀스는 정확히 한
  프레임에만 존재한다. **stdout에 직접 쓰지 말 것**(렌더러가 그 fd를 소유한다. bubbletea v1에는
