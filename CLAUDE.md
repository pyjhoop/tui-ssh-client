...
`docs/V6_plan.md`(암호화 금고·ssh-agent·비공개 레포 동기화, **계획 단계**) 참조 — 구현 결정이 바뀌면 그 문서도 같이 갱신할 것.

## 현재 상태
v0 + v1 + v2 + v3 + v4 + v5 구현 완료. `go build`/`go test -race`/`go vet` 모두 통과한다.
...
**자격증명은 `.gitignore`가 막는다.** `servers.json`은 비밀번호를 평문으로 담고 `keys/`에는
개인키가 들어간다 — 정상 위치는 레포 밖(`~/.config/ssh-client/`)이지만, `XDG_CONFIG_HOME=.`
같은 실행 한 번이면 작업 디렉터리에 떨어진다. 그래서 `servers.json`/`ui.json`/`known_hosts`/
...
바꾸려면 `tabKey()` 한 함수만 고치면 된다. `tabKey`는 `confirm`/`pending`/`rename`/`sftpErr`/
`importing`이 떠 있으면 **아무것도 하지 않는다** — 모달이 모든 키를 먹는다는 규약이 탭 키에도
적용된다(`TestImportSwallowsTabKeys`).
...
- 루트 model의 상태는 `focus`(sidebar|form|session|local|remote|import)와
  `rightMode`(empty|form|terminal|error|sftp|import) 두 축. **키 라우팅은 항상 `focus` 기준**이며,
...
- `internal/ui`는 터미널 없이 root model을 직접 두드린다. 레이아웃 불변식(`TestLayoutAlignment`,
  `TestLayoutAlignmentWithPanels`, `TestLayoutAlignmentWithGroupsAndImport`)은 확인 패널·에러 카드·
  그룹 헤더·필터 입력·import 미리보기 어느 상태에서도 모든 행이 정확히 width여야 하고
  **세로 예산은 그대로**여야 한다.
